...
jQuery(document).ready(function(){
	...
	if(typeof(scsActiveTab) != 'undefined' && scsActiveTab != 'main_page' && jQuery('#toplevel_page_coming-soon-supsystic').hasClass('wp-has-current-submenu')) {
		...
		subMenus.removeClass('current').each(function(){
			if(jQuery(this).find('a[href$="&tab='+ scsActiveTab+ '"]').length) {
	...
	if(jQuery('.scsInputsWithDescrForm').length) {
	...
	if(jQuery('#scsPopupGoToTopBtn').length) {
	...
	if(jQuery('.scsCopyTextCode').length) {
...
function scsInitStickyItem() {
	jQuery(window).scroll(function(){
		...
		if(wasSticking) {
			if(jQuery('#scsPopupGoToTop').length)
...
function scsInitMainPromoPopup() {
	if(!SCS_DATA.isPro) {
		...
		jQuery('.scsProOpt').change(function(e){
			...
			var parent = null;
			if(jQuery(this).parents('#scsPopupMainOscs').length) {
				parent = jQuery(this).parents('label:first');
			} else if(jQuery(this).parents('.scsPopupOptRow:first').length) {
